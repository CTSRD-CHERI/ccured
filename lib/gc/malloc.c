...
/* sm:Allocate lb bytes of data of a given kind */
# ifdef __STDC__
    GC_PTR GC_malloc_kind(size_t lb, int kind)
# else
    GC_PTR GC_malloc_kind(lb, kind)
    size_t lb;
    int kind;
# endif
{
...
#       endif
	...
	FASTLOCK();
        if( EXPECT(!FASTLOCK_SUCCEEDED() || (op = *opp) == 0, 0) ) {
            FASTUNLOCK();
            return(GENERAL_MALLOC((word)lb, kind));
        }
        /* See above comment on signals.	*/
        *opp = obj_link(op);

        /* sm: this is unnecessary if kind == ATOMIC */
        obj_link(op) = 0;
