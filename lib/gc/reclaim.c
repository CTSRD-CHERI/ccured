...
COUNT_DECL
{
	...
	while( p <= plim )  {
		...
		p += sz;
	        ...
	        noticedUnreachable(p, WORDS_TO_BYTES(sz));  /* sm */
		INCR_WORDS(sz);
...
#   define DO_OBJ(start_displ) \
	if (!(mark_word & ((word)1 << start_displ))) { \
            noticedUnreachable(p+start_displ, WORDS_TO_BYTES(2));  /* sm */ \
...
#   define DO_OBJ(start_displ) \
	if (!(mark_word & ((word)1 << start_displ))) { \
            noticedUnreachable(p+start_displ, WORDS_TO_BYTES(4));  /* sm */ \
	    p[start_displ] = (word)list; \
	    list = (ptr_t)(p+start_displ); \
	    p[start_displ+1] = 0; \
...
#   endif
    ...
    /* go through all words in block */
    /* sm: that is, all the words which are the start of potential malloc blocks */
	while( p <= plim )  {
	    /* sm: I thought free list block mark bits were clear at this */
	    /* point, so how does this not claim they are leaks too? */
	    if( !mark_bit_from_hdr(hhdr, word_no) ) {
...
#   define DO_OBJ(start_displ) \
	if (!(mark_word & ((word)1 << start_displ))) { \
            noticedUnreachable(p+start_displ, WORDS_TO_BYTES(1));  /* sm */ \
	    p[start_displ] = (word)list; \
	    list = (ptr_t)(p+start_displ); \
	    INCR_WORDS(1); \
	}
...
hdr * hhdr;             /* sm: corresponding block header */
GC_bool init;
ptr_t list;
word sz;                /* sm: size in words of objects in this block */
COUNT_DECL
{
    ptr_t result = list;

    /* sm: why do we have 8 different static calls to GC_write_hint, */
    /* when we could just have one right before "if(init)" ?? */
...
# endif
{
    ...
    if( sz > MAXOBJSZ ) {  /* 1 big object */
        if( !mark_bit_from_hdr(hhdr, 0) ) {
            /* sm: I think we just noticed an unreachable large block; */
            /* my hope is that 'hbp' is the pointer to the user block */
            noticedUnreachable(hbp, WORDS_TO_BYTES(sz));   /* sm */
...
#	      endif
        ...
        if (report_if_found) {
          ...
          /* sm: we've just found an entire hblock of unreachable blocks; */
          /* are all of these necessarily blocks the programmer malloc'd? */
          /* may as well try to notice something.. */
          noticedContiguousUnreachable(      /* sm */
            hbp,                   /* ptr to start of contiguous blocks */
            WORDS_TO_BYTES(sz),    /* size of each block, in bytes */
            HBLKSIZE);             /* size in bytes of the entire array */
